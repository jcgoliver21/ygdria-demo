...
function openDialog(message,{value,notice=false,choices}={}){
  const task=()=>new Promise(resolve=>{
    ...
    if(choices){input=document.createElement('select');input.name='answer';for(const [id,text] of choices){const option=document.createElement('option');option.value=id;option.textContent=text;input.append(option);}label.append(input);}
    form.append(label);
...
export const askChoice=(message,choices)=>openDialog(message,{choices});
