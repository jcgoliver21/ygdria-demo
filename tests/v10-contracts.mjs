...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=11\.0\.48/);
  assert.match(html,/v10-config\.js\?v=11\.0\.48/);
  assert.match(html,/v10-animations\.js\?v=11\.0\.48/);
  assert.match(html,/humanos-lore-v10\.js\?v=11\.0\.48/);
  assert.match(html,/game-v10\.js\?v=11\.0\.48/);
  assert.match(config,/version:'v11\.0\.48'/);
  assert.match(sw,/12r-v11\.0\.48/);
...
check('v12 fixa a física de escala das ações e a leitura mobile',()=>{
  ...
  assert.match(game,/HUMAN_CHAPTER_BODY_PHYSICS_IDS\.has\(character\?\.id\)\r?\n    \? 1/);
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=11.0.48');
