...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=10\.0\.20/);
  assert.match(html,/v10-config\.js\?v=10\.0\.20/);
  assert.match(html,/v10-animations\.js\?v=10\.0\.20/);
  assert.match(html,/game-v10\.js\?v=10\.0\.20/);
  assert.doesNotMatch(html,/game-v9|styles-v9|v9\.3-config/);
});

check('cache offline da v10 é isolado',()=>{
  assert.match(sw,/12r-v10\.0\.20/);
...
check('Lobo Raivoso é espelhado para encarar o centro',()=>{
  ...
  assert.match(css,/enemyIdleStanceFlip/);
});
...
check('inimigos exclusivos usam folhas reais, impactos e arena viva',()=>{
  for(const id of ['capitao','soldado1','soldado2','sold-bib2','sold-bib3','infantaria','cavalaria','comandante','trono','morto','vulto','slime-cereja','lobo-raivoso','espectro','human-guard','rune-slime','shadow-wolf','cursed-wraith','stone-sentinel','crimson-dragon']){
  ...
  for(const needle of ['ENEMY_ANIMATION_LIBRARY','ENEMY_FRAME_SEQUENCES','actionFrames','function enemyAnimationCharacter','fx-impact-burst','fx-critical-impact','arena-atmosphere','boss-presence-mark']) assert.ok(game.includes(needle)||css.includes(needle),`${needle} ausente`);
...
check('animações corporais não usam crescer ou encolher',()=>{
  ...
  for(const keyframe of ['lungeAttack','lungeAttack9','castRise9','idleBreath','idleBreathFlip','enemySway','enemySwayFlip','enemyIdleStance','enemyWeaponAction','enemySpellGesture','enemyImpactRecoil','golemSpawn']){
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=10.0.20');
