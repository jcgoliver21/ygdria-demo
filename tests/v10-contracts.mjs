...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=10\.0\.57/);
  assert.match(html,/v10-config\.js\?v=10\.0\.57/);
  assert.match(html,/v10-animations\.js\?v=10\.0\.57/);
  assert.match(html,/humanos-lore-v10\.js\?v=10\.0\.57/);
  assert.match(html,/game-v10\.js\?v=10\.0\.57/);
  assert.match(config,/version:'v10\.0\.57'/);
  assert.match(sw,/12r-v10\.0\.57/);
...
check('Markdown editável é a fonte canônica das dez fases humanas',()=>{
  ...
  assert.match(game,/STORY_CAMPAIGN_VERSION='10\.0\.57'/);
});
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=10.0.57');
