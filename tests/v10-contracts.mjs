...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=11\.1\.11/);
  assert.match(html,/v10-config\.js\?v=11\.1\.11/);
  assert.match(html,/v10-animations\.js\?v=11\.1\.11/);
  assert.match(html,/humanos-lore-v10\.js\?v=11\.1\.11/);
  assert.match(html,/game-v10\.js\?v=11\.1\.11/);
  assert.match(config,/version:'v11\.1\.11'/);
  assert.match(sw,/12r-v11\.1\.11/);
  ...
  assert.match(html,/embeddedFinalPreview=params\.get\('preview'\)==='human-final'&&window\.top!==window\.self/);
  assert.doesNotMatch(html,/game-v9|styles-v9|v9\.3-config/);
});

check('Bomba de Cor usa a Estrela do Prisma aprovada',()=>{
  assert.match(css,/\.orb\.power-colorbomb\{[\s\S]*?colorBombPrismBreath/);
  assert.match(css,/\.orb\.power-colorbomb::before\{[\s\S]*?clip-path:polygon\(50% 0/);
  assert.match(css,/\.orb\.power-colorbomb::after\{[\s\S]*?background:#36204d/);
  assert.doesNotMatch(css,/colorBombPulse/);
  assert.match(game,/Cria 1 Estrela de Ygdria no tabuleiro/);
...
check('consumíveis dos Humanos e da Luz têm VFX, inventário migrável e efeitos próprios',()=>{
  const consumiveis=['regulacao','regulacao-bernyce','flor-cerejeira','espadas-lendarias','bencao-eternidade','elixir-divino','luz-protetora','lanca-divina','espelho-ygdria','esperanca'];
  ...
  assert.match(game,/const INVENTORY_CATALOG_VERSION='realm-consumables-v3'/);
  ...
  assert.match(game,/function useHopeItem\(\)/);
  assert.match(game,/const HOPE_MAX=10, HOPE_HOUR_MS=60\*60\*1000/);
  assert.match(game,/function spendStoryHope\(\)/);
  ...
  assert.match(css,/\.hope-badge/);
  assert.match(css,/\.hope-restore-vfx/);
...
check('Modo DEV libera conteúdo sem adulterar o progresso normal e usa o trono pintado',()=>{
  ...
  assert.match(game,/faseIdx>worldAccessLimit\(currentRealmId\(\),prog\)/);
  assert.match(game,/idx>worldAccessLimit\(currentRealmId\(\),prog\)/);
  ...
  assert.match(game,/function triggerHumanFinalePrelude\(/);
  assert.match(game,/function runHumanFinalePreview\(/);
  assert.match(game,/finalSceneActor\('bernyce'\)/);
  assert.match(game,/const victims=ACTIVE\.map/);
  assert.match(game,/victims\.forEach\(setFinaleHeroDefeat\)/);
  assert.match(game,/setFinaleHeroDefeat\('gareth'\)/);
  assert.match(game,/placeFinaleTeleportAt\(scene,'adriel-jovem'\)/);
  assert.match(css,/\.human-final-scene/);
  assert.match(css,/\.human-final-prelude/);
  assert.match(css,/@keyframes humanFinalSkullFlight/);
  assert.match(css,/v16-final-julius-skull\/processed\/sheet-transparent\.png/);
  assert.match(game,/data-final-cinematic/);
...
check('Backstage preserva as dez fases humanas canônicas',()=>{
...
check('opções profissionais estão ligadas à persistência',()=>{
  ...
  assert.ok(html.includes('options-dialog')&&html.includes('options-tabs')&&html.includes('options-accordion'),'abas e acordeões de Opções ausentes');
  assert.ok(css.includes('.options-grid')&&css.includes('.options-tab')&&css.includes('.options-accordion'),'estilo do novo layout de Opções ausente');
  assert.ok(!html.includes('👁 Visualização'),'ícone de olho não deve aparecer em Opções');
});

check('rodapé de jornada acompanha os destinos e sincroniza usuário, Kalegs e Esperança',()=>{
  assert.equal((html.match(/data-realm-footer/g)||[]).length,5,'rodapé deve existir no menu, mapa, Torre, especiais e Minha Conta');
...
check('cronômetros e painéis de consumíveis preservam o fluxo da batalha',()=>{
  assert.match(game,/function beginMissionField\(\)[\s\S]{0,900}?startMissionTimer\(true\);[\s\S]{0,140}?setBattlePhase\('idle'\)/);
  assert.match(game,/function openShopFromMochila\(\)[\s\S]{0,260}?closeMochila\(\)[\s\S]{0,180}?openPanel\('shopScreen'\)/);
  assert.match(game,/function closeShopToMochila\(\)[\s\S]{0,460}?renderMochila\(\)[\s\S]{0,180}?bag\?\.classList\.add\('show'\)/);
  assert.match(game,/async function usarItemBatalha[\s\S]{0,800}?await closeMochilaBeforeItemUse\(\)/);
  assert.ok(!game.includes('Teste público: todas as 10 fases'),'informação interna de teste não pode aparecer na seleção de fases');
  assert.ok(css.includes('.is-extra-long-title')&&css.includes('-webkit-line-clamp:2'),'título de fase precisa ter contenção responsiva');
});
...
check('inimigos usam animação por ação e VFX temporário, sem aura corporal',()=>{
  for(const needle of ['function animateEnemyAvatar','function playEnemyAction','playEnemyAction(idx,\'hit\')','playEnemyAction(idx,\'attack\')','playEnemyAction(enemyIdx,\'cast\')']) assert.ok(game.includes(needle),needle+' ausente');
  for(const selector of ['.unit.ready .avatar-circle{animation:none!important;box-shadow:none!important;cursor:pointer}','.unit-charge-aura{display:none!important}','.enemy-static-avatar[data-action="attack"]']) assert.ok(css.includes(selector),selector+' ausente');
  assert.doesNotMatch(game,/<div class="unit-charge-aura"/,'aura corporal não pode voltar ao HTML da unidade');
});
...
check('Prólogo, Humanos e Torre usam trilhas autorais com troca para o confronto final',()=>{
  for(const file of ['Ygdria_Prologo.mp3','Ygdria_Humanos_1_9.mp3','Ygdria_Batalha_Capitulo_1.mp3','Ygdria_10_Sombras_Que_Devoram.mp3','Ygdria_10_Sombras_Que_Devoram_Final.mp3']){
  ...
  assert.match(game,/tower:\{src:'assets\/audio\/Ygdria_Batalha_Capitulo_1\.mp3'/);
  assert.match(game,/if\(towerMode&&!dailyRunMode\) return 'tower'/);
  assert.match(game,/prepareStageMusic\('tower'\)/);
  ...
  assert.match(game,/function pauseMusic\(\)/);
...
check('HUD R2 aprovado mantém HP lateral, cartas verticais, dois relógios e três estilos de blocos',()=>{
  ...
  assert.match(css,/HUD R2 aprovado/);
  assert.match(css,/board-style-relic/);
  ...
  assert.match(html,/battle-ornate-frame/);
  assert.match(css,/hud-r3-ornate-frame\.png/);
});
...
check('Maril usa folhas corporais aprovadas e efeitos reais de água separados',()=>{
  for(const asset of [
    ...
    'assets/characters/runtime-v10/agua/cast-maril-bubbles-3x2.png',
    'assets/characters/runtime-v10/agua/attack-water-blast-3x2.png',
    'assets/characters/runtime-v10/agua/cast-water-bubbles-3x2.png'
  ]) assert.ok(fs.existsSync(path.join(root,asset)),`folha da Maril ausente: ${asset}`);
  assert.match(animations,/agua\/attack-maril-trident-3x2\.png/);
  assert.match(animations,/agua\/cast-maril-bubbles-3x2\.png/);
  assert.match(game,/heroId==='agua'\) return 'water-jet'/);
  assert.match(game,/maril-water-blast-sheet/);
  assert.match(game,/maril-water-bubble-sheet/);
  assert.match(css,/\.attack-water-jet \.maril-water-blast-sheet/);
  assert.match(css,/\.maril-water-bubble-sheet/);
});

check('aura sustenta a conjuração, coreografa passivas e a galeria replica os VFX aprovados',()=>{
  for(const needle of ['ABILITY_OPPONENT_TYPES','ABILITY_IMMEDIATE_DAMAGE_TYPES','CONJURATION_LEAD_MS','beginHeroConjurationLoop','endHeroConjurationLoop','clearHeroConjurationLoops','conjurationOnly:true','fromSustained','resumeConjuration','launchAbilityAttackPresentation','performEnemyStageAbility','await runStageAbilities()']) assert.ok(game.includes(needle),`${needle} ausente`);
  for(const tipo of ['dotAll','paralisiaTempo','sombrasDevoradoras']) assert.ok(game.includes(`'${tipo}'`),`${tipo} não entra na coreografia de alvo`);
  for(const id of ['motionShowcaseTarget','motionShowcaseAura','motionShowcaseVfx','motionShowcaseImpact']) assert.ok(html.includes(`id="${id}"`),`${id} ausente da galeria`);
  for(const needle of ['motionShowcaseFxSpec','renderMotionShowcaseVfx','motionShowcaseAttackFrames','motionShowcaseAuraFrames']) assert.ok(game.includes(needle)||css.includes(needle),`${needle} ausente da vitrine`);
});
...
check('v12 fixa a física de escala das ações e a leitura mobile',()=>{
  ...
  assert.match(game,/HUMAN_CHAPTER_BODY_PHYSICS_IDS\.has\(character\?\.id\)\s*\? 1 : Number\(ACTION_PHYSICS_SCALE/);
...
check('início revalida a fase narrativa e o HUD de leitura fica fora da arena',()=>{
  assert.match(game,/if\(!orphanStart\) prepareStorySelection\(\);[\s\S]+?if\(!isValidHeroTeam\(chosenIds\)\|\|!chosenIds\.every\(idx=>selectionAvailability\(idx\)\.selectable\)\)/);
  assert.match(game,/const optional=\[\.\.\.new Set\(chosenIds\)\]\.filter\(i=>allowedIdx\.includes\(i\)&&!fixedIdx\.includes\(i\)\);[\s\S]+?chosenIds=\[\.\.\.fixedIdx,\.\.\.optional\]\.slice\(0,4\);/);
  assert.match(game,/function isFixedStoryHero\(idx\)/);
  assert.match(game,/if\(isFixedStoryHero\(idx\)\)\{/);
...
check('Torre possui faixa de chão e perspectiva próprias',()=>{
  assert.match(game,/const TOWER_GROUND=\[0\.57,1\]/);
  assert.match(game,/else if\(towerMode\)\{ \[top,bot\]=TOWER_GROUND; \}/);
  assert.match(game,/towerMode\?' tower-stage':''/);
  assert.match(css,/\.arena\.tower-stage\{--horizon:56%;background-position:center 50%!important\}/);
  assert.match(game,/assets\/bg\/torre-eternidade-andar-v11\.png/);
  assert.ok(fs.existsSync(path.join(root,'assets','bg','torre-eternidade-andar-v11.png')),'cenário exclusivo da Torre ausente');
  assert.match(html,/tower-dialog/);
});
...
check('mapa vertical preserva geografia e expande somente o oceano lateral',()=>{
  ...
  assert.match(game,/class=\"realm-copy\"/);
  assert.match(game,/class=\"realm-copy\"[\s\S]*class=\"realm-subtitle\"[\s\S]*class=\"pin-gem\"/);
  ...
  assert.match(css,/\.map-viewport > #worldScreen \.fase-footer\{/);
  assert.match(css,/\.fase-stars\.rank-facil\{color:#dca566/);
  assert.match(css,/\.fase-stars\.rank-normal\{color:#e2e8f0/);
  assert.match(css,/\.fase-stars\.rank-dificil\{color:#f5c768/);
  assert.match(css,/\.map-viewport > #worldScreen \.fase-stars\.rank-pesadelo\{/);
  assert.match(game,/function nightmareUnlockedForPhase\(/);
  assert.match(game,/class="fase-diff/);
  assert.match(game,/class="fase-footer"/);
  assert.doesNotMatch(game,/class="fase-ranks"/);
  assert.doesNotMatch(html,/id="diffGroup"/);
  ...
  assert.ok(fs.existsSync(path.join(root,'assets','map','ygdria-designer-v2.png')),'arte designer do mapa ausente');
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=11.1.11');
...
check('Torre usa andares únicos, Game Over no tabuleiro e orientações corrigidas',()=>{
  for(const needle of ['const vistos=new Set();','card.nome','type.n','const escala=1+(ciclo*.20);','function showTowerGameOverPanel()','towerGameOverPanel','HERO_DEFAULT_RIGHT_FACING_IDS=new Set([\'bernyce\',\'kalander\',\'jules\',\'aarthas-darke\'])','ENEMY_LEFT_FACING_CARD_IDS=new Set([\'elizier\',\'aarthas-darke\'])','function enemySpriteFlip(e){return Boolean(e?.flip)}']) assert.ok(game.includes(needle),`${needle} ausente do runtime`);
...
check('carregamento de animação é sob demanda e possui fallback',()=>{
  ...
  assert.match(game,/function preloadSpriteBatch\(sources,load,concurrency=2\)/);
  assert.match(game,/const criticalConcurrency=mobileViewport\?2:4/);
  assert.match(game,/image\.fetchPriority=priority/);
  assert.match(game,/function pumpHeroHitMaskWarmup\(/);
  assert.match(game,/heroHitMaskWarmupQueue\.push\(src\)/);
...
check('vitrine humana pré-carrega poses e trata M como conjuração local',()=>{
  for(const needle of ['function preloadSpriteSource(','function preloadLabAction(','readySpriteAssets','pendingSpriteAssets','await preloadLabAction(entry,action)','Conjuração','lab-aura','configureLabAura','conjurationAuraVfx','specialConjurationVfx','conjurationVfxFor','v13-conjuration/aura-runes/processed/sheet-transparent.png','v15-conjuration/special/kalander','v14-conjuration/special/bernyce','v15-conjuration/special/jules','v15-conjuration/special/julius','julius/cast-r5/processed/sheet-transparent.png','webkitMaskImage','dataset.masked','action===\'cast\'&&chapterMode']) assert.ok(animationShowcase.includes(needle),`${needle} ausente da vitrine`);
...
check('Kalegs, recompensas humanas e coleção persistente estão integrados',()=>{
  assert.match(game,/const KALEGS_NAME='Kalegs'/);
  assert.match(game,/function formatKalegs\(value\)/);
  assert.match(game,/const HUMAN_PHASE_REWARDS=Object\.freeze\(/);
  assert.match(game,/facil:\{k:10,card:'gareth'\}/);
  assert.match(game,/pesadelo:\{k:40,card:'jules',items:\{'bencao-eternidade':1\}\}/);
  assert.match(game,/function claimHumanPhaseReward\(fase,diff,\{winner=true\}=\{\}\)/);
  assert.match(game,/12r_human_phase_rewards/);
  assert.match(game,/function cardOwned\(id\)/);
  assert.match(game,/function renderGroupEditor\(\)/);
...
check('premiações usam recibo transacional, recuperação legada e carta navegável',()=>{
  for(const needle of ['HUMAN_REWARD_LEDGER_KEY','function applyHumanPhaseReward(','function reconcileLegacyHumanPhaseRewards()','function humanRewardCardUnlocks()','function reconcileHumanCardUnlocks()','function restoreRewardStorage(','12r_card_unlocks','12r_human_reward_ledger_v2']) assert.ok(game.includes(needle),`${needle} ausente da transação de recompensa`);
  assert.match(game,/if\(rewardLedger\(\)\[key\]\)\{\s*reconcileHumanCardUnlocks\(\);/);
  assert.match(game,/SAVE_EXPORT_EXACT_KEYS=new Set\(\[\s*'12r_ach'.*'12r_card_unlocks'/s);
