...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=10\.0\.46/);
  assert.match(html,/v10-config\.js\?v=10\.0\.46/);
  assert.match(html,/v10-animations\.js\?v=10\.0\.46/);
  assert.match(html,/game-v10\.js\?v=10\.0\.46/);
  assert.match(config,/version:'v10\.0\.46'/);
  assert.match(sw,/12r-v10\.0\.46/);
...
check('v13 limita o grid perspectivado à Cidade das Cerejeiras',()=>{
  ...
  assert.match(css,/\.arena\.perspective-physics \.unit-ground-shadow/);
});

check('passagem gráfica v10.0.46 mantém física e dá resposta ao combate',()=>{
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=10.0.46');
