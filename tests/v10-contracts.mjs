...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=11\.0\.45/);
  assert.match(html,/v10-config\.js\?v=11\.0\.45/);
  assert.match(html,/v10-animations\.js\?v=11\.0\.45/);
  assert.match(html,/humanos-lore-v10\.js\?v=11\.0\.45/);
  assert.match(html,/game-v10\.js\?v=11\.0\.45/);
  assert.match(config,/version:'v11\.0\.45'/);
  assert.match(sw,/12r-v11\.0\.45/);
...
check('mapa vertical preserva geografia e expande somente o oceano lateral',()=>{
  ...
  assert.match(game,/class=\"realm-copy\"[\s\S]*class=\"pin-gem\"/);
  ...
  assert.match(css,/\.map-art \.realm-subtitle\{[\s\S]*font-size:clamp\(13px,1\.24cqw,16px\)/);
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=11.0.45');
