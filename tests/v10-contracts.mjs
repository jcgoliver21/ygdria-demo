...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=10\.0\.30/);
  assert.match(html,/v10-config\.js\?v=10\.0\.30/);
  assert.match(html,/v10-animations\.js\?v=10\.0\.30/);
  assert.match(html,/game-v10\.js\?v=10\.0\.30/);
  assert.doesNotMatch(html,/game-v9|styles-v9|v9\.3-config/);
});

check('cache offline da v10 é isolado',()=>{
  assert.match(sw,/12r-v10\.0\.30/);
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=10.0.30');
