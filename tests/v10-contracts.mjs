...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=10\.0\.44/);
  assert.match(html,/v10-config\.js\?v=10\.0\.44/);
  assert.match(html,/v10-animations\.js\?v=10\.0\.44/);
  assert.match(html,/game-v10\.js\?v=10\.0\.44/);
  assert.match(config,/version:'v10\.0\.44'/);
  assert.match(sw,/12r-v10\.0\.44/);
...
check('v13 limita o grid perspectivado à Cidade das Cerejeiras',()=>{
  ...
  assert.match(game,/const largestFootprint=Math\.max\(88/);
  assert.match(game,/return unitWidth\*\.76; \/\/ mesma largura da sombra de contato/);
  assert.match(game,/const candidateCell=floorHeight\/weights\.reduce/);
  assert.match(game,/class="physical-floor-row"/);
  assert.match(game,/data-grid-slot/);
  assert.match(css,/\.physical-floor-grid\{[\s\S]+?top:var\(--floor-top/);
  assert.match(css,/\.physical-floor-row\{[\s\S]+?grid-template-columns:repeat\(var\(--floor-columns\)/);
  assert.match(css,/aspect-ratio:1 \/ 1/);
  assert.match(css,/perspective\(1200px\) rotateX\(18deg\) scaleY\(1\.06\)/);
});

check('passagem gráfica v10.0.44 mantém física e dá resposta ao combate',()=>{
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=10.0.44');
