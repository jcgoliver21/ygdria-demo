...
check('arquivos públicos apontam somente para v10',()=>{
  assert.match(html,/styles-v10\.css\?v=10\.0\.40/);
  assert.match(html,/v10-config\.js\?v=10\.0\.40/);
  assert.match(html,/v10-animations\.js\?v=10\.0\.40/);
  assert.match(html,/game-v10\.js\?v=10\.0\.40/);
  assert.match(config,/version:'v10\.0\.40'/);
  assert.match(sw,/12r-v10\.0\.40/);
...
check('v13 limita o grid perspectivado à Cidade das Cerejeiras',()=>{
  ...
  assert.match(css,/grid-template-columns:repeat\(3,var\(--grid-hero-cell\)\)/);
  assert.match(css,/grid-template-columns:repeat\(2,var\(--grid-enemy-cell\)\)/);
  assert.match(css,/aspect-ratio:1 \/ 1/);
  assert.match(css,/perspective\(780px\) rotateX\(56deg\)/);
});

check('passagem gráfica v10.0.40 mantém física e dá resposta ao combate',()=>{
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v10.js?v=10.0.40');
