...
check('arquivos públicos apontam somente para v9.3',()=>{
  assert.match(html,/styles-v9\.3\.css\?v=9\.3\.20/);
  assert.match(html,/v9\.3-config\.js\?v=9\.3\.20/);
  assert.match(html,/game-v9\.3\.js\?v=9\.3\.20/);
  assert.doesNotMatch(html+sw,/game-v9\.1|styles-v9\.1|spritehud2/);
});

check('cache offline da v9.3 é isolado',()=>{
  assert.match(sw,/12r-v9\.3\.20/);
...
check('menu inicial não bloqueia o primeiro toque',()=>{
  ...
  const deferredGame=html.indexOf('game-v9.3.js?v=9.3.20');
