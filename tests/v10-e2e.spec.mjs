...
test('v11 mostra dez formações e seleciona pelo corpo visível sem setas',async({page})=>{
  ...
  await page.setViewportSize({width:390,height:844});
  await page.evaluate(()=>{
    saveCardUnlocks(KINGDOMS.slice(0,4).map(hero=>hero.id));
  ...
  await page.evaluate(()=>onHeroAvatarClick(ACTIVE[0]));
...
test('final humano encena prólogo, quedas e teleporte antes do epílogo canônico',async({page})=>{
  const errors=await boot(page,'flow');
  await page.setViewportSize({width:390,height:844});
  /* Encadeamento físico: depois da missão de Cedric + Jules, Cedric permanece
     no plano distante durante a batalha de Bernyce + Kalander. */
  await page.evaluate(()=>{
    worldRun={active:true,fase:9,nivel:4,storyMode:false};
    chosenIds=['adriel-jovem','gareth','roland','elizier'].map(id=>KINGDOMS.findIndex(hero=>hero.id===id));
    beginGame(0); skipStory();
  });
  await expect(page.locator('.royal-court-cedric-distant')).toBeVisible();
  const cedricLeadIn=await page.evaluate(()=>{
    const cedric=document.querySelector('.royal-court-cedric-distant');
    const arena=document.querySelector('#arena');
    const r=cedric.getBoundingClientRect(),a=arena.getBoundingClientRect();
    return {left:r.left-a.left,bottom:a.bottom-r.bottom,width:r.width};
  });
  expect(cedricLeadIn.left).toBeGreaterThan(190);
  expect(cedricLeadIn.bottom).toBeGreaterThan(150);
  expect(cedricLeadIn.width).toBeGreaterThan(40);
  /* A cena da missão 5 não pode inventar outra formação para Bernyce e
     Kalander. Registramos os retângulos da própria missão 10/4 e exigimos que
     as cópias do prólogo e do epílogo ocupem as mesmas âncoras no chão. */
  const courtBattleAnchors=await page.evaluate(()=>{
    const arena=document.querySelector('#arena').getBoundingClientRect();
    const read=id=>{
      const r=document.querySelector(id).getBoundingClientRect();
      return {left:Math.round(r.left-arena.left),bottom:Math.round(arena.bottom-r.bottom),width:Math.round(r.width)};
    };
    return {kalander:read('#enemy-0'),bernyce:read('#enemy-1')};
  });
  await page.evaluate(()=>{
    worldRun={active:true,fase:9,nivel:5,storyMode:false};
    chosenIds=['adriel-jovem','gareth','roland','elizier'].map(id=>KINGDOMS.findIndex(hero=>hero.id===id));
  ...
  await expect(page.locator('.royal-court-jules')).toBeVisible();
  await expect(page.locator('.royal-court-cedric')).toHaveCount(0);
  ...
  const juliusForeground=await page.evaluate(()=>{
  ...
  /* Bernyce e Kalander permanecem nas âncoras da luta anterior; como Julius
     entra em primeiro plano, a leitura é aferida pela camada visível, não por
     uma falsa troca de posição dos corpos. As cópias da cena devem ter o
     mesmo porte-base dos inimigos adultos da arena. */
  const preludePositions=await page.evaluate(()=>{
    const arena=document.querySelector('#arena').getBoundingClientRect();
    const measure=id=>{
      const el=document.querySelector(`.human-final-prelude .finale-${id}`);
      const r=el.getBoundingClientRect();
      return {left:Math.round(r.left-arena.left),bottom:Math.round(arena.bottom-r.bottom),width:Math.round(r.width)};
    };
    return {bernyce:measure('bernyce'),kalander:measure('kalander'),cedric:measure('cedric')};
  });
  expect(Math.abs(preludePositions.bernyce.left-courtBattleAnchors.bernyce.left)).toBeLessThanOrEqual(4);
  expect(Math.abs(preludePositions.bernyce.bottom-courtBattleAnchors.bernyce.bottom)).toBeLessThanOrEqual(4);
  expect(Math.abs(preludePositions.kalander.left-courtBattleAnchors.kalander.left)).toBeLessThanOrEqual(4);
  expect(Math.abs(preludePositions.kalander.bottom-courtBattleAnchors.kalander.bottom)).toBeLessThanOrEqual(4);
  ...
  const finalPositions=await page.evaluate(()=>{
    const arena=document.querySelector('#arena').getBoundingClientRect();
    const measure=id=>{
      const el=document.querySelector(`.human-final-scene .finale-${id}`);
      const r=el.getBoundingClientRect();
      return {left:Math.round(r.left-arena.left),bottom:Math.round(arena.bottom-r.bottom),width:Math.round(r.width)};
    };
    return {bernyce:measure('bernyce'),kalander:measure('kalander'),cedric:measure('cedric')};
  ...
  const finaleScaleAudit=await page.evaluate(()=>Object.fromEntries(['bernyce','kalander','cedric','julius'].map(id=>{
    const variant=id==='julius'?'original':'fallen';
    const el=document.querySelector(`.human-final-scene [data-finale-actor="${id}-${variant}"]`);
    return [id,Math.round(el?.getBoundingClientRect().width||0)];
  })));
  expect(finaleScaleAudit.bernyce).toBeGreaterThanOrEqual(100);
  expect(finaleScaleAudit.kalander).toBeGreaterThanOrEqual(100);
  expect(finaleScaleAudit.cedric).toBeGreaterThanOrEqual(60);
  expect(finaleScaleAudit.julius).toBeGreaterThanOrEqual(90);
...
test('volumes NaN são saneados e cliques de áudio não geram pageerror',async({page})=>{
  await page.addInitScript(()=>{
    ...
    localStorage.setItem('12r_stage_music_volume','NaN');
  ...
  for(const selector of ['#volumeRange','#musicVolumeRange','#stageMusicVolumeRange','#sfxVolumeRange']){
...
test('Fase 10 usa as duas trilhas licenciadas com mixagem protegida',async({page})=>{
  ...
  const audio=await page.evaluate(()=>{
    ...
    chosenIds=['adriel-jovem','gareth','roland','elizier'].map(id=>KINGDOMS.findIndex(hero=>hero.id===id));
    beginGame(0); skipStory();
    const baseTrack=stageMusicActive;
    ...
    playLicensedStageMusic('base');
    const resume={sameVoice:baseTrack?.audio===voiceBeforePause,paused:baseTrack?.paused};
    worldRun.nivel=5;
  ...
  expect(audio.loop).toMatchObject({handoff:true,continuous:true,loop:false,crossfade:.8,loopOut:.95});
...
test('seleção de história mostra só o elenco da missão; modo livre guarda o restante em Selecionar Heróis',async({page})=>{
  ...
  for(const layout of [result.story.layout,result.free.layout]){
    expect(layout).toMatchObject({screenOverflow:0,gridOverflow:0,startVisible:true,captionCount:0,bonusRadius:'0px',bonusAligned:true,subAlign:'left'});
  }
...
test('PWA abre o núcleo v10 sem rede depois da instalação',async({page,context})=>{
  ...
  expect(registration.caches).toContain('12r-v11.0.78');
  try{
...
test.describe('@production publicação real',()=>{
  ...
  test('gate público abre e o jogo autenticado completa o fluxo principal',async({page})=>{
    ...
    await expect(page.locator('body')).toHaveAttribute('data-game-ready','1');
    await expect(page.locator('#menuVersion')).toContainText('VERSÃO 11');
  await expect.poll(()=>page.evaluate(()=>window.YGDRIA_V10?.version)).toBe('v11.0.78');
