...
function renderStatusTray(){
  ...
  const detail=(label)=>{
    ...
    if(/cegueira|blind|ceguera/.test(s)) return T('PrÃ³ximo ataque inimigo falha.','The next enemy attack misses.','El prÃ³ximo ataque enemigo falla.');
    if(/defesa rompida|defense broken|defensa rota/.test(s)) return T('Inimigos recebem dano aumentado.','Enemies take increased damage.','Los enemigos reciben daÃ±o aumentado.');
    if(/incinerar|incinerate/.test(s)) return T('Ataques acumulam dano contÃ­nuo por 3 turnos.','Attacks add damage-over-time for 3 turns.','Los ataques aÃ±aden daÃ±o continuo por 3 turnos.');
    if(/dano contÃ­nuo|damage over time/.test(s)) return T('Dano aplicado no fim de cada turno.','Damage is applied at the end of each turn.','DaÃ±o aplicado al final de cada turno.');
    return '';
  };
...
let tempoSombrioTimer=null;    /* Julius inimigo · corrupção por tempo real */
...
function loadStage(idx){
  ...
  if(worldRun.active && pendingDimensional.length){
    ...
    if(somaPct>0){
      ...
      setTimeout(()=>setBattleStatus(T(`⚔ As Sombras Dimensionais feriram os inimigos em ${somaPct}%!`,`⚔ The Dimensional Shadows wounded the enemies by ${somaPct}%!`,`⚔ ¡Las Sombras Dimensionales hirieron a los enemigos en un ${somaPct}%!`),'support'),900);
    }
  }
  ...
  if(enemies.some(e2=>e2.cardId==='julius'&&e2.hp>0)){
    tempoSombrioTimer=setInterval(()=>{
      ...
      { let hud=document.getElementById('sombraHud');
        ...
        hud.textContent='🌑 '+corrompidas+'/'+(SIZE*SIZE); }
      if(!hasValidMoves()) shuffleBoard(false);
...
function triggerAbility(idx, a, options={}){
  ...
  if(!options.deferRoomCheck&&allEnemiesDefeated()){
    window.setTimeout(()=>finishRoomIfCleared(T(`${L(a.name)} derrotou todos os inimigos da sala.`,`${L(a.name)} defeated every enemy in the room.`,`${L(a.name)} derrotó a todos los enemigos de la sala.`)),40);
  }
}
