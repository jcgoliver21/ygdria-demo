...
function armStageMusicLoop(track,voice){
  ...
  const loopOut=Number(track.loopOut)||STAGE10_LOOP_CROSSFADE;
  if(!Number.isFinite(duration)||duration<=loopOut) return;
  const wait=Math.max(80,(duration-voice.audio.currentTime-loopOut)*1000);
...
function createStageMusicVoice(track){
  ...
  audio.addEventListener('timeupdate',()=>{
    ...
    if(Number.isFinite(remaining)&&remaining<=Number(track.loopOut||STAGE10_LOOP_CROSSFADE)) queueStageMusicLoop(track,voice);
  });
...
function queueStageMusicLoop(track,voice,force=false){
  ...
  if(!force&&Number.isFinite(remaining)&&remaining>Number(track.loopOut||STAGE10_LOOP_CROSSFADE)){ armStageMusicLoop(track,voice); return false; }
...
const LEGACY_SHOP_ITEMS=[
  ...
  {id:'prisma', uso:'batalha', nome:'Prisma Selvagem', desc:'Cria 1 Estrela de Ygdria no tabuleiro.', preco:160, icon:'🌈', en:{nome:'Wild Prism', desc:'Creates 1 Star of Ygdria on the board.'}, es:{nome:'Prisma Salvaje', desc:'Crea 1 Estrella de Ygdria en el tablero.'}},
...
I18N_DICT['Estrela de Ygdria']=['Star of Ygdria','Estrella de Ygdria'];
...
const STATIC_I18N=[
  ['#playBtn .menu-label','<span class="menu-title-line">História</span><small class="menu-hint">Leia as Crônicas dos 12 Reinos</small>','<span class="menu-title-line">Story</span><small class="menu-hint">Read the Chronicles of the 12 Realms</small>','<span class="menu-title-line">Historia</span><small class="menu-hint">Lee las Crónicas de los 12 Reinos</small>'],
  ['#galleryBtn .menu-label','<span class="menu-title-line">Biblioteca da Eternidade</span><small class="menu-hint">Crônicas, criaturas, NPCs e cartas</small>','<span class="menu-title-line">Library of Eternity</span><small class="menu-hint">Chronicles, creatures, NPCs and cards</small>','<span class="menu-title-line">Biblioteca de la Eternidad</span><small class="menu-hint">Crónicas, criaturas, NPCs y cartas</small>'],
  ['#shopBtn .menu-label','<span class="menu-title-line">Mercado Central dos Reinos</span><small class="menu-hint">Itens, poções e preparos</small>','<span class="menu-title-line">Central Market of the Realms</span><small class="menu-hint">Items, potions and supplies</small>','<span class="menu-title-line">Mercado Central de los Reinos</span><small class="menu-hint">Objetos, pociones y preparativos</small>'],
  ['#achBtn .menu-label','<span class="menu-title-line">Perfil</span><small class="menu-hint" id="profileHint">Sua lenda, conquistas e jornada</small>','<span class="menu-title-line">Profile</span><small class="menu-hint" id="profileHint">Your legend, achievements and journey</small>','<span class="menu-title-line">Perfil</span><small class="menu-hint" id="profileHint">Tu leyenda, logros y jornada</small>'],
  ['#optionsBtn .menu-label','<span class="menu-title-line">Opções</span><small class="menu-hint">Som, imagem e acessibilidade</small>','<span class="menu-title-line">Options</span><small class="menu-hint">Sound, display and accessibility</small>','<span class="menu-title-line">Opciones</span><small class="menu-hint">Sonido, imagen y accesibilidad</small>'],
  ...
  ["#helpScreen .tutorial-step:nth-child(7)","<b>7. Crie power-ups</b><p>Combine 4 para criar uma esfera listrada, 5 para uma Estrela de Ygdria e forme T ou L para uma esfera encapsulada.</p>","<b>7. Create power-ups</b><p>Match 4 to create a striped orb, 5 for a Star of Ygdria, and make a T or L shape for a wrapped orb.</p>","<b>7. Crea potenciadores</b><p>Combina 4 para crear una esfera rayada, 5 para una Estrella de Ygdria y forma una T o L para una esfera envuelta.</p>"],
...
const COACH_STEPS_I18N={
  ...
  en:[
    ...
    {text:'Match 4+ spheres to craft power-ups: striped sweep lines, wrapped explode areas and Stars of Ygdria clear colors.'},
...
function renderCellContent(cell,r,c){
  ...
  if(colorIdx===-2 && power?.type==='colorBomb'){
    ...
    wrapper.setAttribute('aria-label',T(`Estrela de Ygdria, linha ${r+1}, coluna ${c+1}`,`Star of Ygdria, row ${r+1}, column ${c+1}`,`Estrella de Ygdria, fila ${r+1}, columna ${c+1}`));
...
/* Estrela de Ygdria + outro especial: o especial não é apenas um alcance maior.
...
function buildPowerComboResolution(from,to,powerFrom,powerTo){
  ...
  if(hasColorBomb){
    if(powerFrom?.type==='colorBomb'&&powerTo?.type==='colorBomb'){
      return {label:T('Dupla Estrela de Ygdria: todo o tabuleiro foi purificado!','Double Star of Ygdria: the entire board was purified!','¡Doble Estrella de Ygdria: todo el tablero fue purificado!'),cells:Array.from({length:SIZE*SIZE},(_,i)=>({r:Math.floor(i/SIZE),c:i%SIZE})),auditKind:'double-color-bomb'};
    }
    ...
    if(otherPower?.type==='striped'){
      ...
      return {label:T('Estrela de Ygdria + Listrado: todas as joias da cor viraram listrados e dispararam!','Star of Ygdria + Striped: every gem of that color became striped and fired!','¡Estrella de Ygdria + Rayado: todas las joyas del color se volvieron rayadas y dispararon!'),cells:uniqueCells(cells),transforms};
    }
    if(otherPower?.type==='wrapped'){
      const transforms=targets.map(cell=>({...cell,power:{type:'wrapped'}}));
      return {label:T('Estrela de Ygdria + Embrulhado: todas as joias da cor viraram embrulhados e explodiram!','Star of Ygdria + Wrapped: every gem of that color became wrapped and exploded!','¡Estrella de Ygdria + Envuelto: todas las joyas del color se volvieron envueltas y explotaron!'),cells:uniqueCells(cells),transforms};
    }
    return {label:T('Estrela de Ygdria: todas as joias da cor escolhida foram removidas!','Star of Ygdria: every gem of the chosen color was removed!','¡Estrella de Ygdria: todas las joyas del color elegido fueron eliminadas!'),cells:uniqueCells(cells)};
  }
...
async function resolveMatches(){
  ...
  if(createdPlans.length){
    const names=createdPlans.map(plan=>plan.power.type==='colorBomb'?T('Estrela de Ygdria','Star of Ygdria','Estrella de Ygdria'):plan.power.type==='wrapped'?T('Embrulhado','Wrapped','Envuelto'):T('Listrado','Striped','Rayado'));
...
function triggerAbility(idx, a, options={}){
  ...
  switch(a.tipo){
    ...
    case 'spawnColorBombs':
      ...
      setBattleStatus(T(`${L(k.nome)} inscreveu ${a.quantidade||3} Estrelas de Ygdria no tabuleiro.`,`${L(k.nome)} inscribed ${a.quantidade||3} Stars of Ygdria on the board.`,`${L(k.nome)} inscribió ${a.quantidade||3} Estrellas de Ygdria en el tablero.`),'support');
      break;
    case 'summonHarpies': {
  ...
  function renderTowerScreen(){
    ...
    if(box){
      ...
      box.innerHTML='<div class="tower-ledger">'
        ...
        +'<section class="tower-monthly-ranking" aria-label="'+T('Ranking mensal','Monthly ranking','Ranking mensual')+'"><div class="tower-ranking-heading"><span>'+T('RANKING MENSAL','MONTHLY RANKING','RANKING MENSUAL')+'</span><small>'+mk+' · '+T('em breve com o banco de dados','coming with the database','próximamente con la base de datos')+'</small></div><div class="tower-ranking-placeholder"><span class="tower-ranking-crown" aria-hidden="true">♛</span><div><b>'+T('A sua escalada poderá entrar para a história.','Your climb can enter history.','Tu ascenso podrá entrar en la historia.')+'</b><p>'+T('Este espaço exibirá os melhores aventureiros do mês quando o ranking online for ativado.','This space will show the month\'s best adventurers when the online ranking is activated.','Este espacio mostrará los mejores aventureros del mes cuando se active el ranking en línea.')+'</p></div><span class="tower-ranking-place"># —</span></div></section>'
